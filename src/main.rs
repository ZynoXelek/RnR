...
struct Args {
    ...
    #[arg(
        short = 'c',
        long = "code_gen",
        help = "Generate the backend ASM code (requires virtual machine execution)"
    )]
    code_gen: bool,
...
fn parse_prog_from_file(path: &str) -> Result<Prog, io::Error> {
    ...
    match file {
        ...
        Err(e) => {
            eprintln!(" * File parsing: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn ast_subcommand(prog: Prog, path: &str) -> io::Result<()> {
    ...
    match result {
        ...
        Err(e) => {
            eprintln!(" * AST dump: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn type_check_subcommand(prog: Prog) -> Result<Prog, TypeError> {
    ...
    match checked_prog {
        ...
        Err(e) => {
            eprintln!(" * Type check: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn optimize_subcommand(prog: Prog) -> Result<Prog, Error> {
    ...
    match opt_prog {
        ...
        Err(e) => {
            eprintln!(" * Optimization: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn vm_subcommand(prog: Prog) -> Result<(), EvalError> {
    ...
    match prog_type {
        ...
        Err(e) => {
            eprintln!(" * VM evaluation: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn code_gen_subcommand(prog: Prog) -> Result<Instrs, Error> {
    ...
    match instrs {
        ...
        Err(e) => {
            eprintln!(" * Code generation: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn asm_subcommand(instrs: Instrs, path: &str) -> io::Result<()> {
    ...
    match result {
        ...
        Err(e) => {
            eprintln!(" * ASM dump: {}{}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
...
fn run_subcommand(mips: &mut Mips) -> Result<(), MipsError> {
    ...
    match result {
        ...
        Err(e) => {
            eprintln!(" * Running code: {}{:?}{}", RED_COLOR, e, DEFAULT_COLOR);
            Err(e)
        }
    }
}
    ...
    if let Some(asm_input) = &args.asm_input {
        ...
        if args.type_check {
            ...
            if args.virtual_machine {
                println!("\n ----------------- ");
                match vm_subcommand(prog.clone()) {
                    Ok(_) => {}
                    Err(_) => {
                        eprintln!(
                            "{}Error{}: VM execution failed, aborting final steps {}now{}.",
                            RED_COLOR, DEFAULT_COLOR, RED_COLOR, DEFAULT_COLOR
                        );
                        return;
                    }
                };
