...
mod test_parsing {
    ...
    mod test_parse_lit {
        ...
        #[test]
        fn parse_lit_array_empty() {
            let lit: Literal = parse("[]");
            let expected: Literal = Array::new(vec![]).into();
            assert_eq!(lit, expected);
        }

        #[cfg(test)]
        pub mod test_parse_lit_array {
            use super::*;

            #[test]
            fn parse_lit_array_1() {
                let lit: Literal = parse("[1, 2, 3]");
                let expected: Literal = Array::new(vec![
            ...
            #[test]
            fn parse_lit_array_alt() {
                let lit: Literal = parse("[1; 2]");
                let expected: Literal =
                    Array::new(vec![Expr::Lit(Literal::Int(1)), Expr::Lit(Literal::Int(1))]).into();
                assert_eq!(lit, expected);
            }

            #[test]
            fn parse_lit_array_3() {
            ...
            #[test]
            fn parse_lit_array_4() {
                let lit: Literal = parse("[\"hello\", \"world\"]");
                let expected: Literal = Array::new(vec![
                    Expr::Lit(Literal::String("hello".to_string())),
                    Expr::Lit(Literal::String("world".to_string())),
                ])
                .into();
                assert_eq!(lit, expected);
            }
            ...
            #[test]
            fn parse_lit_array_of_arrays() {
                let lit: Literal = parse("[[1, 2], [3; 4]]");
                let expected: Literal = Array::new(vec![
                    Array::new(vec![Expr::Lit(Literal::Int(1)), Expr::Lit(Literal::Int(2))]).into(),
                    Array::new(vec![
                        Expr::Lit(Literal::Int(3)),
                        Expr::Lit(Literal::Int(3)),
                        Expr::Lit(Literal::Int(3)),
            ...
            #[test]
            fn parse_array_with_expressions_alt_1() {
                let lit: Literal = parse("[7 * 5; 5 - 1]");
                let init_expr = Expr::bin_op(
                    BinOp::Mul,
            ...
            #[test]
            fn parse_array_with_expressions_alt_2() {
                let lit: Literal = parse("[a[1]; 2 * 3]");
                let init_expr = Expr::bin_op(
                    BinOp::Get,
        ...
        #[test]
        fn test_array_parsing_errors() {
            assert_parse_fail::<Literal>("[1, 2, 3");
            assert_parse_fail::<Literal>("[, 2, 3]");
            assert_parse_fail::<Literal>("[1, 2 3]"); // Missing comma
            assert_parse_fail::<Literal>("[2; 3 + 4"); // Missing closing bracket
            assert_parse_fail::<Literal>("[2; 3, 2]"); // Using ; instead of ,
            assert_parse_fail::<Literal>("[2, 3; 2]"); // Using ; instead of ,
            assert_parse_fail::<Literal>("[3; a]"); // Using non constant integer expression for size
            assert_parse_fail::<Literal>("[3; a[0]]"); // Using non constant integer expression for size
            assert_parse_fail::<Literal>("[3; true]"); // Using non constant integer expression for size
        }
    }
    ...
    mod test_parse_expr {
        ...
        fn array_access_expr_with_array() {
            ...
            let expected = {
                ...
                .into();
                let array_idx =
                    Array::new(vec![Expr::Lit(Literal::Int(1)), Expr::Lit(Literal::Int(0))]).into();
    ...
    mod test_parse_statement {
        ...
        #[test]
        fn test_statement_array_assign_1() {
        ...
        #[test]
        fn test_statement_array_assign_2() {
            let stmt: Statement = parse("a[0][2] = false");
            let expected = Statement::Assign(
                Expr::bin_op(
                    BinOp::Get,
                    Expr::bin_op(
    ...
    mod test_parse_block {
        ...
        fn test_block2() {
            let ts: proc_macro2::TokenStream =
                "{ let b : bool = false; b = true }".parse().unwrap();
            let bl: Block = syn::parse2(ts).unwrap();
            println!("bl {:?}", bl);
            println!("bl (pretty printing)\n{}", bl);
